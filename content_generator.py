...
def generate_tweet_parts(app_info):
    ...
    try:
        ...
        『{app_info.get('アプリ名', '')}』を使ってみた私の正直レポ✍️
        ...
        ## 絶対的なルール
        1.  上記のテンプレートにあなたの生成パーツをすべて合体させた **最終的なツイート全体の文字数が、絶対に140文字を超えないようにしてください。**
        2.  文字数計算の際、 `🔗 (URL)` の部分は **23文字** としてカウントしてください。
        3.  この文字数制限を守るために、あなたが生成する `kyokan_tweet`, `good_point`, `caution_point` の長さを賢く調整してください。
        ...
        - "caution_point": アプリの注意点をリアルな視点で紹介。
        - "hashtags": 「#PR」を必ず含め、投稿に合うハッシュタグを3つまとめた配列。
